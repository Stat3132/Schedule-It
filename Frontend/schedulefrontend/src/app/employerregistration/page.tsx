...
import {
  ...
  const [message, setMessage] = useState<{ type: 'success' | 'error' | 'info'; text: string } | null>(null);
    ...
    useEffect(() => {
      const remembered = loadRememberedEmail("employer");
      if (remembered?.email) {
    ...
    useEffect(() => {
      if (!remember) {
        clearRememberedEmail("employer");
        return;
      }
      if (formData.email.trim()) {
        saveRememberedEmail("employer", formData.email);
      }
    ...
    async function handlePasswordReset() {
      ...
      try {
        ...
        console.error('Employer reset error:', error);
    ...
    if (profile?.role !== 'employer') {
      await supabase.auth.signOut();
      setMessage({ type: 'error', text: 'Wrong login. Employee role has been detected.' });
      setLoading(false);
      return;
    }
          ...
          <form className="space-y-4" onSubmit={signIn}>
            ...
            {showResetForm && (
              <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm space-y-2">
                <Label htmlFor="employer-reset-email">Send reset link to</Label>
                <Input
            ...
            <p className="mt-6 text-center text-sm text-muted-foreground">
              ...
              <Link href="/employeronboarding/userinfo" className="text-primary hover:underline">
